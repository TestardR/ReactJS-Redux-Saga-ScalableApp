...
function* fetchTopics() {
  try {
    const topics = yield call(fetchTopicsFromServer); // get data from server
    yield put(requestTopicsSucceeded(topics)); // put data from server in state
  } catch (e) {
    yield put(requestTopicsFailed(topics));
  }
}
...
export default [fetchTopicsSaga, selectTopicSaga];
